...
(function () {
  ...
  try { firebase.initializeApp(firebaseConfig); } catch (e) { console.error('Firebase:', e); }
  ...
  function detectPetSize() {
    ...
    const tinyB = ['чіхуахуа','той-тер','той тер','йорк','йоркшир','мальтезе','мальтійськ','папійон','ши-тцу','ши тцу','шитцу','померан','шпіц мініатюр'];
    const smallB = ['шпіц','мопс','такса','пекінес','французький бульдог','кокер','бігль','бішон','карликов','цвергшнауцер','вест хайленд','джек рассел','корги','шелті','бостон тер'];
    const medB = ['бордер колі','стафорд','пітбуль','англійський бульдог','шарпей','далматин','хаскі','самоїд','австралійська вівчарка','спанієль','пойнтер','сеттер'];
    const largeB = ['лабрадор','ретрівер','вівчарка','ротвейлер','доберман','боксер','рідж','курцхаар','малінуа','бельгійська','колі','грейхаунд','акіта','кане корсо','кане-корсо'];
    const giantB = ['дог','мастиф','сенбернар','ньюфаундленд','бернський','леонбергер','ірландський вовкодав','тибетський мастиф','алабай','кавказ'];
    if (tinyB.some(b => breed.includes(b))) return 'tiny';
    if (smallB.some(b => breed.includes(b))) return 'small';
    if (medB.some(b => breed.includes(b))) return 'medium';
    if (largeB.some(b => breed.includes(b))) return 'large';
    if (giantB.some(b => breed.includes(b))) return 'giant';
    return 'medium';
  }
  function getSizeLabel() { return { tiny:'мініатюрна (до 7 кг)', small:'маленька (7–12 кг)', medium:'середня (12–25 кг)', large:'велика (25–40 кг)', giant:'гігантська (40+ кг)' }[detectPetSize()] || 'середня'; }
  function getSpayAgeRange() { return { tiny:{min:5,max:7,label:'5–7 міс'}, small:{min:6,max:8,label:'6–8 міс'}, medium:{min:8,max:12,label:'8–12 міс'}, large:{min:12,max:18,label:'12–18 міс'}, giant:{min:18,max:24,label:'18–24 міс'} }[detectPetSize()] || {min:8,max:12,label:'8–12 міс'}; }
  function getNeuterAgeRange() { return { tiny:{min:6,max:8,label:'6–8 міс'}, small:{min:6,max:9,label:'6–9 міс'}, medium:{min:9,max:12,label:'9–12 міс'}, large:{min:12,max:18,label:'12–18 міс'}, giant:{min:18,max:24,label:'18–24 міс'} }[detectPetSize()] || {min:9,max:12,label:'9–12 міс'}; }

  // ===== Streak =====
  function updateStreak() {
    const today = todayKey();
    const yesterday = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
    const todayHas = eventsState.some(e => { const ts = tsToDate(e.createdAt); return ts && ts >= startOfToday(); });
  ...
  function renderStreak() {
    updateStreak();
    const badge = $('streakBadge'); const card = $('streakCard');
    if (streakData.count > 0) {
      if (badge) { show(badge); $('streakCount').textContent = streakData.count; }
      if (card) { show(card); $('streakText').textContent = `${streakData.count} ${streakData.count === 1 ? 'день' : streakData.count < 5 ? 'дні' : 'днів'} поспіль!`; $('streakSub').textContent = streakData.count >= 7 ? '🏆 Тижневий рекорд!' : streakData.count >= 3 ? '💪 Чудова послідовність!' : 'Так тримати!'; }
  ...
  function renderHeader() {
    const name = currentPet?.name?.trim() || 'Песик';
    const weeks = getAgeInWeeks(currentPet?.birthDate);
    $('petNameHeader').textContent = name;
    $('headerSub').textContent = `${weekLabel(weeks)} · ${getProgramByAge(weeks).stage}`;
    $('profileName').textContent = name;
    $('profileMeta').textContent = [currentPet?.breed || 'Порода?', weekLabel(weeks), currentPet?.sex || ''].filter(Boolean).join(' · ');
    const av = $('userAvatar'); if (av) av.innerHTML = currentUser?.photoURL ? `<img src="${currentUser.photoURL}" alt="">` : avatarLetter(currentUser?.displayName || name);
  }

  // ===== RENDER: Daily Tip =====
  function renderDailyTip() {
    const el = $('dailyTipText'); if (!el) return;
    const weeks = getAgeInWeeks(currentPet?.birthDate); const sex = currentPet?.sex || '';
    const last7 = eventsState.filter(e => { const ts = tsToDate(e.createdAt); return ts && ts >= new Date(Date.now() - 7*86400000); });
    const s7 = last7.filter(e => isToiletSuccess(e.eventType)).length; const m7 = last7.filter(e => isToiletMiss(e.eventType)).length;
    const t7 = s7+m7; const rate = t7 > 0 ? Math.round(s7/t7*100) : null;
    const tr7 = last7.filter(e => e.eventType === 'training').length;

    const tips = [];
    if (rate !== null) { if (rate >= 90) tips.push(`🎉 ${rate}% горшик за тиждень! Молодці!`); else if (rate >= 70) tips.push(`📈 Горшик ${rate}% — прогрес!`); else if (rate >= 40) tips.push(`💪 Горшик ${rate}%. Частіше виводьте!`); else if (t7 > 3) tips.push(`🎯 Горшик ${rate}%. Менше простору + суперласощі!`); }
    if (t7 === 0 && eventsState.length < 5) tips.push('📝 Записуйте туалет — через 3 дні побачите патерн!');
    if (tr7 === 0) tips.push('🎓 0 тренувань! Навіть 2 хв/день = результат 💪');
    ...
    el.textContent = tips.length > 0 ? tips[new Date().getHours() < 12 ? 0 : Math.min(1, tips.length-1)] : (pool[new Date().getDate() % pool.length]?.text || 'Записуйте події! 📊');
  }

  // ===== RENDER: KPIs =====
  function renderKpis() {
    const start = startOfToday(); const todayEv = eventsState.filter(e => { const ts = tsToDate(e.createdAt); return ts && ts >= start; });
    const s = todayEv.filter(e => isToiletSuccess(e.eventType)).length; const m = todayEv.filter(e => isToiletMiss(e.eventType)).length;
  ...
  function renderOneTap() {
    const grid = $('onetapGrid'); if (!grid) return;
    const items = [
    ...
    grid.innerHTML = items.map(i => `<button type="button" class="onetap-btn ${i.cls}" data-onetap="${i.type}"><span class="onetap-icon">${i.icon}</span>${i.label}</button>`).join('');
    $$('[data-onetap]').forEach(btn => { btn.addEventListener('click', async () => { if (btn.classList.contains('logged')) return; btn.classList.add('logged'); haptic(); await addEvent({ eventType: btn.dataset.onetap, timeLabel: nowTime() }); setTimeout(() => btn.classList.remove('logged'), 2000); }); });
  }

  // ===== RENDER: Weekly Report =====
  function renderWeeklyReport() {
    const card = $('weeklyReport'); const content = $('weeklyContent'); if (!card || !content) return;
    const dismissed = localStorage.getItem('dc_weekly_dismissed') === todayKey();
    if (eventsState.length < 5 || dismissed) { hide(card); return; }
    const now = new Date(); const twStart = new Date(now); twStart.setDate(now.getDate()-7); twStart.setHours(0,0,0,0);
    const lwStart = new Date(twStart); lwStart.setDate(lwStart.getDate()-7);
    const tw = eventsState.filter(e => { const ts = tsToDate(e.createdAt); return ts && ts >= twStart; });
    const lw = eventsState.filter(e => { const ts = tsToDate(e.createdAt); return ts && ts >= lwStart && ts < twStart; });
    if (tw.length < 3) { hide(card); return; }
    const tws = tw.filter(e => isToiletSuccess(e.eventType)).length; const twm = tw.filter(e => isToiletMiss(e.eventType)).length;
    const twt = tws+twm; const twRate = twt > 0 ? Math.round(tws/twt*100) : null;
    const lws = lw.filter(e => isToiletSuccess(e.eventType)).length; const lwm = lw.filter(e => isToiletMiss(e.eventType)).length;
    const lwt = lws+lwm; const lwRate = lwt > 0 ? Math.round(lws/lwt*100) : null;
    const twTr = tw.filter(e => e.eventType === 'training').length; const lwTr = lw.filter(e => e.eventType === 'training').length;
    const twW = tw.filter(e => e.eventType === 'walk').length;
    function ch(c, p) { if (p == null || c == null) return ''; const d = c-p; if (d > 0) return `<span class="ws-change up">+${d}↑</span>`; if (d < 0) return `<span class="ws-change down">${d}↓</span>`; return ''; }
    show(card);
    content.innerHTML = `<div class="weekly-stat"><span class="ws-label">📊 Подій</span><span class="ws-value">${tw.length}${ch(tw.length,lw.length)}</span></div>${twRate!==null?`<div class="weekly-stat"><span class="ws-label">🚽 Горшик</span><span class="ws-value">${twRate}%${ch(twRate,lwRate)}</span></div>`:''}<div class="weekly-stat"><span class="ws-label">🎓 Тренувань</span><span class="ws-value">${twTr}${ch(twTr,lwTr)}</span></div><div class="weekly-stat"><span class="ws-label">🚶 Прогулянок</span><span class="ws-value">${twW}</span></div><div class="weekly-stat"><span class="ws-label">🔥 Streak</span><span class="ws-value">${streakData.count} дн.</span></div>${twRate!==null&&twRate>=80?'<p style="margin-top:0.5rem;font-size:0.85rem;color:var(--success)">🎉 Чудовий тиждень!</p>':''}`;
  }

  // ===== RENDER: AI Plan =====
  async function generateAIPlan() {
    const card = $('aiPlanCard'); const content = $('aiPlanContent'); if (!card || !content) return;
    if (!currentPet?.name) { hide(card); return; }
    const cached = localStorage.getItem('dc_aiplan');
    if (cached) { try { const p = JSON.parse(cached); if (p.date === todayKey() && p.plan) { show(card); content.innerHTML = p.plan; return; } } catch {} }
    show(card); content.innerHTML = '<p class="text-muted">🧠 Генерую план...</p>';
    const weeks = getAgeInWeeks(currentPet?.birthDate); const issues = currentPet?.issues || '';
    const last7 = eventsState.filter(e => { const ts = tsToDate(e.createdAt); return ts && ts >= new Date(Date.now()-7*86400000); });
    const s7 = last7.filter(e => isToiletSuccess(e.eventType)).length; const m7 = last7.filter(e => isToiletMiss(e.eventType)).length;
    const rate = (s7+m7) > 0 ? Math.round(s7/(s7+m7)*100) : null;
    ...
    try { const r = await fetchAIResponse(prompt); const html = r.split('\n').filter(l => l.trim()).map(l => `<div class="ai-plan-item">${l}</div>`).join(''); content.innerHTML = html || '<p class="text-muted">Спробуйте 🔄</p>'; localStorage.setItem('dc_aiplan', JSON.stringify({ date: todayKey(), plan: html })); }
    catch { content.innerHTML = '<p class="text-muted">Помилка. Натисніть 🔄</p>'; }
  }
  ...
  function renderHeatInfo() {
    ...
    if (currentPet?.sex === 'хлопчик') { card.style.display=''; if(field)field.style.display='none'; const range=getNeuterAgeRange(); let h=`<div class="plan-item"><strong>✂️ Кастрація</strong><br>📏 ${sizeLabel}<br>📅 ${range.label}</div>`; if(monthsAge!=null){if(monthsAge<range.min-1)h+=`<div class="plan-item">🕐 ${monthsAge} міс — рано.</div>`;else if(monthsAge>=range.min-1&&monthsAge<=range.max)h+=`<div class="plan-item" style="color:var(--accent)">✅ ${monthsAge} міс — час!</div>`;else h+=`<div class="plan-item">ℹ️ Можна будь-коли.</div>`;} h+=`<details style="margin-top:0.75rem"><summary style="cursor:pointer;font-weight:600;font-size:0.85rem">ℹ️ Деталі</summary><div style="margin-top:0.5rem"><div class="plan-item">✅ Менше маркування, агресії, втікань</div><div class="plan-item">⚠️ Може набрати вагу</div></div></details><p class="text-muted" style="margin-top:0.5rem;font-size:0.78rem">⚠️ Рішення — з ветеринаром.</p>`; info.innerHTML=h; return; }
    ...
    let expFirst={tiny:6,small:7,medium:10,large:12,giant:16}[size]||10; let h='';
    if(lastHeat){const next=new Date(new Date(lastHeat).getTime()+HEAT_INFO.avgCycleDays*86400000);const du=daysBetween(new Date(),next);if(du>30)h+=`<div class="plan-item">📅 Наступна ~${next.toLocaleDateString('uk')} (${du} дн.) 😌</div>`;else if(du>0)h+=`<div class="plan-item" style="color:var(--warning)">⚠️ Тічка через ~${du} днів!</div>`;else h+=`<div class="plan-item" style="color:var(--danger)">🩸 Можливо зараз!</div>`;}
    else if(weeks==null)h+='<p class="text-muted">Вкажіть дату народження 📅</p>';
    else{const until=expFirst-monthsAge;if(monthsAge>=20)h+=`<div class="plan-item">❓ Не зафіксована. Ветеринар?</div>`;else if(until<=1)h+=`<div class="plan-item" style="color:var(--warning)">⚠️ Перша скоро! (${monthsAge} міс)</div>`;else if(until<=3)h+=`<div class="plan-item">📅 Через ~${until} міс</div>`;else h+=`<div class="plan-item">🕐 Ще далеко (~${expFirst} міс)</div>`;h+=`<div class="plan-item"><strong>🔍 Ознаки:</strong><br>• Набрякла петля • Виділення • Вилизування</div>`;}
    h+=`<details style="margin-top:0.75rem"><summary style="cursor:pointer;font-weight:600;font-size:0.85rem">📖 Фази</summary><div style="margin-top:0.5rem">${HEAT_INFO.phases.map(p=>`<div class="plan-item"><strong>${p.name}</strong> (${p.days})<br>${p.desc}</div>`).join('')}</div></details>`;
    let sp=`<div class="plan-item">📏 ${sizeLabel} · 📅 ${spayRange.label}</div><div class="plan-item" style="color:var(--danger)">🚫 Не під час тічки!</div>`;
    if(monthsAge!=null&&!lastHeat&&monthsAge>=spayRange.min-1&&monthsAge<=spayRange.max&&(size==='tiny'||size==='small'))sp+=`<div class="plan-item" style="color:var(--accent)">✅ Зараз — час!</div>`;
    if(lastHeat){const ds=daysBetween(new Date(lastHeat),new Date());if(ds>=60&&ds<=120)sp+=`<div class="plan-item" style="color:var(--accent)">✅ Безпечно (${ds} дн. після)</div>`;else if(ds<60)sp+=`<div class="plan-item">⏳ Ще ~${60-ds} дн.</div>`;}
  ...
  function renderWeight() {
    ...
    const ds=diff?(diff>0?`+${diff} кг ↑`:diff<0?`${diff} кг ↓`:'='):'';
    ...
    c.innerHTML=html;requestAnimationFrame(()=>renderWeightChart(we));
  }
  ...
  function renderWeightChart(we) {
    ...
    ctx.strokeStyle=gc;ctx.lineWidth=1;for(let i=0;i<=3;i++){const y=pad.top+(i/3)*ch;ctx.beginPath();ctx.moveTo(pad.left,y);ctx.lineTo(w-pad.right,y);ctx.stroke();ctx.fillStyle=tc;ctx.font='10px system-ui';ctx.textAlign='right';ctx.fillText((mx-(i/3)*rng).toFixed(1),pad.left-4,y+3);}
  ...
  function fillPetForm() {
    ...
    if ($('petIssues')) $('petIssues').value = currentPet?.issues || '';
  ...
  // ===== Auth =====
  async function loginGoogle() { showLoading(); try { await auth.signInWithPopup(googleProvider); } catch (e) { if (e.code==='auth/popup-blocked'||e.code==='auth/popup-closed-by-user') { try { await auth.signInWithRedirect(googleProvider); } catch (err) { toast(err.message||'Помилка','error'); } } else if (e.code==='auth/unauthorized-domain') toast('Домен не авторизовано','error'); else toast(e.message||'Помилка','error'); } finally { hideLoading(); } }
  async function logout() { unsubEvents?.(); unsubMembers?.(); unsubPet?.(); unsubEvents=unsubMembers=unsubPet=null; await auth.signOut(); currentUser=null; workspaceId=null; workspaceData=null; currentPet=null; eventsState=[]; membersState=[]; hide($('appContent')); show($('authScreen')); }
  ...
  function setOnboardingStep(step) { $$('.onboarding-step').forEach(s => s.classList.add('hidden')); show($(`onboardingStep${step}`)); $$('.ob-dot').forEach(d => d.classList.toggle('active', parseInt(d.dataset.step) === step)); }

  function bindOnboarding() {
    $('obNext1')?.addEventListener('click', () => { if (!$('obName').value.trim()) { toast('Введіть ім\'я 🐾','error'); return; } setOnboardingStep(2); });
