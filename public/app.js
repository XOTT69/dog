...
function renderSuggestion() {
  ...
  const missTriggers = {};
  toilet.filter(e => e.eventType === 'miss' && e.trigger).forEach(e => {
...
function renderMembers() {
  ...
  list.innerHTML = membersState.length
    ? membersState.map(m => `<div class="member-chip"><div class="m-avatar">${m.photoURL ? `<img src="${m.photoURL}" alt="${m.displayName || 'user'}">` : avatarLetter(m.displayName)}</div><span>${m.displayName || 'Учасник'}</span></div>`).join('')
    : '<div class="empty">Поки що тут тільки ви.</div>';
}

function renderWorkspaceMeta() {
  const nameEl = $('workspaceName');
  if (nameEl) nameEl.textContent = workspaceData?.name || '—';
...
function renderFeed(targetId = 'recentLogs') {
  ...
  list.innerHTML = eventsState.slice(0, 20).map(item => {
    ...
    return `<div class="feed-item" data-event-id="${item.id}"><div><strong>${conf.icon} ${conf.label}</strong><div class="meta">${timeSince(item.createdAt)}${item.note ? ` · ${item.note}` : ''}</div></div><button type="button" class="btn-sm" data-delete-event="${item.id}">Видалити</button></div>`;
  }).join('');
...
function openSheet() {
  setVisible($('eventSheet'), true);
  $('eventTime').value = nowTime();
}

function closeSheet() {
  setVisible($('eventSheet'), false);
}
...
function bindOnboarding() {
  ...
  $$('.ob-option').forEach(btn => btn.addEventListener('click', () => {
  ...
  $('obFinish')?.addEventListener('click', async () => {
    ...
    toast('Початок збережено', 'success');
  });
}
...
function bindEvents() {
  ...
  window.addEventListener('resize', () => {
    if ($('tabDiary')?.classList.contains('active')) renderChart('progressChartDiary');
  });
}
