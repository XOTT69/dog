...
(function () {
  ...
  var AGE_PROGRAMS = window.AGE_PROGRAMS;
  var COURSES = window.COURSES;
  var KNOWLEDGE = window.KNOWLEDGE;
  var SOCIAL_ITEMS = window.SOCIAL_ITEMS;
  var TOILET_GUIDE = window.TOILET_GUIDE;
  var TYPE_CONFIG = window.TYPE_CONFIG;
  var EVENT_CATEGORIES = window.EVENT_CATEGORIES;
  var DAILY_TIPS = window.DAILY_TIPS;
  var HEAT_INFO = window.HEAT_INFO;
  ...
  var eventsState = [], membersState = [];
  var currentCourseId = 'pee-pad', currentCourseLevel = 'all', currentDiaryFilter = 'all';
  var selectedEventType = null, selectedSheetCategory = 'toilet';
  var unsubEvents = null, unsubMembers = null, unsubPet = null;
  var themeMode = localStorage.getItem('dc_theme') || 'light';
  var dailyDone = JSON.parse(localStorage.getItem('dc_daily') || '{}');
  var streakData = JSON.parse(localStorage.getItem('dc_streak') || '{"count":0,"lastDate":""}');
  var renderQueued = false, activeTab = 'tabHome';
  ...
  function weekLabel(w) { if (w==null) return '—'; if (w<8) return w+' тиж.'; if (w<52) return Math.floor(w/4.345)+' міс.'; var y=w/52; return y<2?y.toFixed(1)+' р.':Math.floor(y)+' р.'; }
  function getProgramByAge(w) { if (w==null) return AGE_PROGRAMS[1]||AGE_PROGRAMS[0]; return AGE_PROGRAMS.find(function(p){return w>=p.minWeeks&&w<p.maxWeeks;})||AGE_PROGRAMS[AGE_PROGRAMS.length-1]; }
  ...
  function detectPetSize() {
    ...
    var breed = ((currentPet&&currentPet.breed)||'').toLowerCase();
    ...
    if (['чіхуахуа','той','йорк','мальтезе','папійон','ши-тцу','померан'].some(function(b){return breed.indexOf(b)>=0;})) return 'tiny';
    if (['шпіц','мопс','такса','пекінес','бігль','корги','джек рассел'].some(function(b){return breed.indexOf(b)>=0;})) return 'small';
    if (['хаскі','стафорд','далматин','шарпей','самоїд'].some(function(b){return breed.indexOf(b)>=0;})) return 'medium';
    if (['лабрадор','ретрівер','вівчарка','ротвейлер','доберман','акіта'].some(function(b){return breed.indexOf(b)>=0;})) return 'large';
    if (['дог','мастиф','сенбернар','бернський','алабай','кавказ'].some(function(b){return breed.indexOf(b)>=0;})) return 'giant';
    return 'medium';
  }
  function getSizeLabel() { return {tiny:'мініатюрна',small:'маленька',medium:'середня',large:'велика',giant:'гігантська'}[detectPetSize()]||'середня'; }
  function getSpayRange() { return {tiny:{l:'5–7 міс'},small:{l:'6–8 міс'},medium:{l:'8–12 міс'},large:{l:'12–18 міс'},giant:{l:'18–24 міс'}}[detectPetSize()]||{l:'8–12 міс'}; }
  function getNeuterRange() { return {tiny:{l:'6–8 міс'},small:{l:'6–9 міс'},medium:{l:'9–12 міс'},large:{l:'12–18 міс'},giant:{l:'18–24 міс'}}[detectPetSize()]||{l:'9–12 міс'}; }

  function updateStreak() {
    var today = todayKey(), yesterday = new Date(Date.now()-86400000).toISOString().slice(0,10);
    var has = eventsState.some(function(e){var ts=tsToDate(e.createdAt);return ts&&ts>=startOfToday();});
    if (has) { if(streakData.lastDate===today) return; streakData.count = streakData.lastDate===yesterday ? streakData.count+1 : 1; streakData.lastDate=today; }
  ...
  function renderStreak() {
    updateStreak(); var b=$('streakBadge'),c=$('streakCard');
    if (streakData.count>0) { if(b){show(b);$('streakCount').textContent=streakData.count;} if(c){show(c);$('streakText').textContent=streakData.count+(streakData.count===1?' день':streakData.count<5?' дні':' днів')+' поспіль!';$('streakSub').textContent=streakData.count>=7?'🏆 Рекорд!':streakData.count>=3?'💪 Чудово!':'Так тримати!';} }
    else { if(b)hide(b); if(c)hide(c); }
  }

  function toast(msg,type) { var box=$('toastContainer'); if(!box)return; var el=document.createElement('div'); el.className='toast '+(type||''); el.textContent=msg; box.appendChild(el); requestAnimationFrame(function(){el.classList.add('show');}); setTimeout(function(){el.classList.remove('show');setTimeout(function(){el.remove();},300);},2800); }
  function setTheme(m) { themeMode=m==='dark'?'dark':'light'; document.documentElement.setAttribute('data-theme',themeMode); localStorage.setItem('dc_theme',themeMode); }
  ...
  function renderHeader() {
    ...
    $('petNameHeader').textContent=name; $('headerSub').textContent=weekLabel(weeks)+' · '+getProgramByAge(weeks).stage;
    $('profileName').textContent=name; $('profileMeta').textContent=[(currentPet&&currentPet.breed)||'',weekLabel(weeks)].filter(Boolean).join(' · ');
  ...
  function renderDailyTip() {
    ...
    var weeks=getAgeInWeeks(currentPet&&currentPet.birthDate), sex=(currentPet&&currentPet.sex)||'';
    ...
    var t7=s7+m7, rate=t7>0?Math.round(s7/t7*100):null;
    var tips=[];
    if(rate!==null){if(rate>=90)tips.push('🎉 '+rate+'% горшик!');else if(rate>=70)tips.push('📈 Горшик '+rate+'%');else if(rate<50&&t7>3)tips.push('🎯 Горшик '+rate+'%. Менше простору!');}
    if(t7===0&&eventsState.length<5)tips.push('📝 Записуйте туалет!');
    ...
    el.textContent = tips.length>0 ? tips[0] : ((pool[new Date().getDate()%pool.length]||{}).text||'Записуйте події!');
  }
  ...
  function renderKpis() {
    ...
    var t=s+m, pct=t>0?Math.round(s/t*100):0;
  ...
  function renderOneTap() {
    ...
    var items=[{type:'pee_success',icon:'💛',label:'Пописяла ✓',cls:'success'},{type:'pee_miss',icon:'💛',label:'Пописяла мимо',cls:'danger'},{type:'poo_success',icon:'💩',label:'Покакала ✓',cls:'success'},{type:'poo_miss',icon:'💩',label:'Покакала мимо',cls:'danger'},{type:'training',icon:'🎓',label:'Тренування',cls:''},{type:'walk',icon:'🚶',label:'Прогулянка',cls:''}];
    ...
    $$('[data-onetap]').forEach(function(btn){btn.addEventListener('click',function(){if(btn.classList.contains('logged'))return;btn.classList.add('logged');haptic();addEvent({eventType:btn.dataset.onetap,timeLabel:nowTime()});setTimeout(function(){btn.classList.remove('logged');},2000);});});
  }
  ...
  function renderWeeklyReport() {
    ...
    var now=new Date(),twS=new Date(now);twS.setDate(now.getDate()-7);twS.setHours(0,0,0,0);
    var lwS=new Date(twS);lwS.setDate(lwS.getDate()-7);
    var tw=eventsState.filter(function(e){var ts=tsToDate(e.createdAt);return ts&&ts>=twS;});
    var lw=eventsState.filter(function(e){var ts=tsToDate(e.createdAt);return ts&&ts>=lwS&&ts<twS;});
    ...
    var twt=tws+twm,twR=twt>0?Math.round(tws/twt*100):null;
    show(card);
    content.innerHTML='<div class="weekly-stat"><span class="ws-label">📊 Подій</span><span class="ws-value">'+tw.length+'</span></div>'+(twR!==null?'<div class="weekly-stat"><span class="ws-label">🚽 Горшик</span><span class="ws-value">'+twR+'%</span></div>':'')+'<div class="weekly-stat"><span class="ws-label">🔥 Streak</span><span class="ws-value">'+streakData.count+' дн.</span></div>';
  }

  function generateAIPlan() {
    var card=$('aiPlanCard'),content=$('aiPlanContent'); if(!card||!content)return;
    ...
    show(card);content.innerHTML='<p class="text-muted">🧠 Генерую...</p>';
    var weeks=getAgeInWeeks(currentPet.birthDate),issues=currentPet.issues||'';
    var prompt='План на СЬОГОДНІ для '+currentPet.name+' ('+weekLabel(weeks)+', '+(currentPet.breed||'?')+', '+getSizeLabel()+')'+(issues?'\nПроблеми: '+issues:'')+'\nДай 4-5 конкретних пунктів. Формат: 1. [коли] — [що]';
  ...
  function renderHeatInfo() {
    var info=$('heatInfo'); if(!info) return;
    var weeks=getAgeInWeeks(currentPet&&currentPet.birthDate);
    var monthsAge=weeks!=null?Math.round(weeks/4.345):null;
    if(currentPet&&currentPet.sex==='хлопчик') { info.innerHTML='<div class="plan-item"><strong>✂️ Кастрація:</strong> '+getSizeLabel()+' · '+getNeuterRange().l+'</div><p class="text-muted" style="font-size:0.78rem">⚠️ Рішення — з ветеринаром.</p>'; return; }
    if(!currentPet||currentPet.sex!=='дівчинка') { info.innerHTML=''; return; }
    var lastHeat=currentPet.lastHeat,expFirst={tiny:6,small:7,medium:10,large:12,giant:16}[detectPetSize()]||10;
    var h='';
    if(lastHeat){var next=new Date(new Date(lastHeat).getTime()+HEAT_INFO.avgCycleDays*86400000);var du=daysBetween(new Date(),next);if(du>30)h+='<div class="plan-item">📅 Наступна ~'+next.toLocaleDateString('uk')+'</div>';else if(du>0)h+='<div class="plan-item" style="color:var(--warning)">⚠️ Через ~'+du+' днів</div>';else h+='<div class="plan-item" style="color:var(--danger)">🩸 Можливо зараз</div>';}
    else if(monthsAge!=null){var until=expFirst-monthsAge;if(until<=1)h+='<div class="plan-item" style="color:var(--warning)">⚠️ Перша скоро</div>';else if(until<=3)h+='<div class="plan-item">📅 Через ~'+until+' міс</div>';else h+='<div class="plan-item">🕐 Ще далеко (~'+expFirst+' міс)</div>';}
    h+='<div class="plan-item"><strong>✂️ Стерилізація:</strong> '+getSizeLabel()+' · '+getSpayRange().l+'</div>';
    h+='<p class="text-muted" style="font-size:0.78rem">⚠️ Рішення — з ветеринаром.</p>';
    info.innerHTML=h;
  }
  ...
  function renderReminders() {
    var list=$('remindersList'); if(!list) return;
    var rem=(currentPet&&currentPet.reminders)||[];
    if(!rem.length){list.innerHTML='<p class="text-muted">Поки немає нагадувань</p>';return;}
    var now=new Date();
    list.innerHTML=rem.map(function(r,i){var d=new Date(r.nextDate);var days=daysBetween(now,d);var cls='',txt='';if(days<0){cls='danger';txt='Прострочено!';}else if(days===0){cls='warning';txt='Сьогодні!';}else if(days<=3){cls='warning';txt='Через '+days+' дн.';}else{txt=d.toLocaleDateString('uk');}return '<div class="feed-item"><div><strong>'+r.label+'</strong><div class="meta '+cls+'">'+txt+'</div></div><button type="button" class="btn btn-ghost btn-sm" data-del-reminder="'+i+'">✕</button></div>';}).join('');
    $$('[data-del-reminder]').forEach(function(btn){btn.addEventListener('click',function(){var idx=parseInt(btn.dataset.delReminder);var rem2=(currentPet.reminders||[]).slice();rem2.splice(idx,1);savePetProfile({reminders:rem2});});});
  }

  function renderRecommendedCourses() {
    var card=$('recommendedCourses'),list=$('recommendedList'); if(!card||!list) return;
    var weeks=getAgeInWeeks(currentPet&&currentPet.birthDate);
    var issues=(currentPet&&currentPet.issues)||'';
    var rec=[];
    if(weeks!=null&&weeks<16) rec=COURSES.filter(function(c){return c.level==='базовий';}).slice(0,3);
    else if(weeks!=null&&weeks<72) rec=COURSES.filter(function(c){return c.level==='середній';}).slice(0,3);
    else rec=COURSES.filter(function(c){return c.level==='просунутий';}).slice(0,3);
    if(issues.indexOf('кусає')>=0||issues.indexOf('гризе')>=0) { var bc=COURSES.find(function(c){return c.id==='bite-control';}); if(bc&&rec.indexOf(bc)<0) rec.unshift(bc); }
    if(issues.indexOf('тягне')>=0||issues.indexOf('повідок')>=0) { var lw=COURSES.find(function(c){return c.id==='leash-walking';}); if(lw&&rec.indexOf(lw)<0) rec.unshift(lw); }
    if(issues.indexOf('гавка')>=0) { var re=COURSES.find(function(c){return c.id==='reactivity';}); if(re&&rec.indexOf(re)<0) rec.unshift(re); }
    rec=rec.slice(0,4);
    if(!rec.length){card.style.display='none';return;}
    card.style.display='';
    list.innerHTML=rec.map(function(c){return '<button type="button" class="course-btn" data-rec-course="'+c.id+'"><span class="c-badge">'+c.badge+'</span><strong>'+c.title+'</strong></button>';}).join('');
  ...
  function renderDiarySummary() {
    var el=$('diarySummary'); if(!el) return;
    var last7=eventsState.filter(function(e){var ts=tsToDate(e.createdAt);return ts&&ts>=new Date(Date.now()-7*86400000);});
    var s=last7.filter(function(e){return isToiletSuccess(e.eventType);}).length;
    var m=last7.filter(function(e){return isToiletMiss(e.eventType);}).length;
    var t=s+m,rate=t>0?Math.round(s/t*100):null;
    var tr=last7.filter(function(e){return e.eventType==='training';}).length;
    var wk=last7.filter(function(e){return e.eventType==='walk';}).length;
    el.innerHTML='<div class="weekly-stat"><span class="ws-label">За 7 днів</span><span class="ws-value">'+last7.length+' подій</span></div>'+(rate!==null?'<div class="weekly-stat"><span class="ws-label">🚽 Горшик</span><span class="ws-value">'+rate+'%</span></div>':'')+'<div class="weekly-stat"><span class="ws-label">🎓 Тренувань</span><span class="ws-value">'+tr+'</span></div><div class="weekly-stat"><span class="ws-label">🚶 Прогулянок</span><span class="ws-value">'+wk+'</span></div>';
  }

  function renderWeight() {
    var c=$('weightHistory'); if(!c) return;
    var we=eventsState.filter(function(e){return e.eventType==='weight'&&e.value;}).slice(0,10).reverse();
    if(!we.length){c.innerHTML='<p class="text-muted">+ → Здоров\'я → ⚖️ Вага</p>';return;}
    var latest=we[we.length-1];
    c.innerHTML='<div class="plan-item"><strong>⚖️ '+latest.value+' кг</strong></div>'+we.slice().reverse().slice(0,3).map(function(e){var d=tsToDate(e.createdAt);return '<div style="display:flex;justify-content:space-between;padding:0.25rem 0;font-size:0.8rem;color:var(--text-secondary)"><span>'+(d?d.toLocaleDateString('uk'):'')+'</span><strong>'+e.value+' кг</strong></div>';}).join('');
  }
  function renderFeed(targetId, filter) {
    filter = filter || 'all';
    var list = $(targetId); if (!list) return;
    var filtered = eventsState;
    if (filter !== 'all') { var cat = EVENT_CATEGORIES.find(function(c){return c.id===filter;}); if (cat) { var types = cat.events.map(function(e){return e.type;}); filtered = eventsState.filter(function(e){return types.indexOf(e.eventType)>=0;}); } }
    // On home — show only today, max 4
    var maxItems = targetId === 'recentLogs' ? 4 : 50;
    if (targetId === 'recentLogs') { var start = startOfToday(); filtered = filtered.filter(function(e){var ts=tsToDate(e.createdAt);return ts&&ts>=start;}); }
    if (!filtered.length) { list.innerHTML = '<div class="empty">'+(targetId==='recentLogs'?'Сьогодні ще немає записів':'Немає записів')+'</div>'; return; }
    list.innerHTML = filtered.slice(0, maxItems).map(function(item) {
      var conf = TYPE_CONFIG[item.eventType] || {icon:'•',label:'Подія'};
      var d = tsToDate(item.createdAt);
      var timeStr = d ? d.toLocaleString('uk',{hour:'2-digit',minute:'2-digit'}) : '';
    ...
    $$('#'+targetId+' [data-delete-event]').forEach(function(btn){btn.addEventListener('click',function(){if(!confirm('Видалити?'))return;deleteEvent(btn.dataset.deleteEvent);});});
  }
  ...
  function renderChart(canvasId) {
    ...
    var days=[]; for(var i=13;i>=0;i--){var d=new Date();d.setDate(d.getDate()-i);d.setHours(0,0,0,0);var next=new Date(d);next.setDate(next.getDate()+1);var dayEv=eventsState.filter(function(e){var ts=tsToDate(e.createdAt);return ts&&ts>=d&&ts<next;});var s=dayEv.filter(function(e){return isToiletSuccess(e.eventType);}).length;var m=dayEv.filter(function(e){return isToiletMiss(e.eventType);}).length;var t=s+m;days.push({date:d,pct:t?Math.round(s/t*100):null});}
    var isDark=themeMode==='dark';var accent=isDark?'#2dd4bf':'#0f766e';var danger=isDark?'#f87171':'#dc2626';var warning=isDark?'#fbbf24':'#d97706';var muted=isDark?'#78716c':'#a8a29e';var border=isDark?'#292524':'#e7e5e4';
    var p={top:10,right:4,bottom:20,left:4};var cw=w-p.left-p.right,ch=h-p.top-p.bottom,bw=cw/days.length;
    ctx.strokeStyle=border;ctx.lineWidth=1;[0,50,100].forEach(function(v){var y=p.top+ch-(v/100)*ch;ctx.beginPath();ctx.moveTo(p.left,y);ctx.lineTo(w-p.right,y);ctx.stroke();});
    days.forEach(function(day,i){var x=p.left+i*bw+bw*0.2,barW=bw*0.6;if(day.pct==null){ctx.fillStyle=muted;ctx.beginPath();ctx.arc(x+barW/2,p.top+ch-3,2,0,Math.PI*2);ctx.fill();}else{var barH=Math.max(3,(day.pct/100)*ch),y2=p.top+ch-barH;ctx.fillStyle=day.pct>=70?accent:day.pct>=40?warning:danger;var r=Math.min(3,barW/2);ctx.beginPath();ctx.moveTo(x,y2+barH);ctx.lineTo(x,y2+r);ctx.quadraticCurveTo(x,y2,x+r,y2);ctx.lineTo(x+barW-r,y2);ctx.quadraticCurveTo(x+barW,y2,x+barW,y2+r);ctx.lineTo(x+barW,y2+barH);ctx.closePath();ctx.fill();}if(i%3===0||i===days.length-1){ctx.fillStyle=muted;ctx.font='10px system-ui';ctx.textAlign='center';ctx.fillText(day.date.getDate()+'/'+(day.date.getMonth()+1),x+barW/2,h-4);}});
  }

  function renderCourses() {
    var grid=$('courseGrid'),viewer=$('selectedCourse'); if(!grid||!viewer) return;
  ...
  function renderMembers(){var list=$('membersList');if(!list)return;list.innerHTML=membersState.length?membersState.map(function(m){return '<div class="member-chip"><div class="m-avatar">'+(m.photoURL?'<img src="'+m.photoURL+'" alt="">':avatarLetter(m.displayName))+'</div><span>'+(m.displayName||'Учасник')+'</span></div>';}).join(''):'<div class="empty">Тільки ви</div>';}
  ...
  function renderAll() {
    ...
    renderHeatInfo();renderAgeFocus();renderReminders();renderRecommendedCourses();renderDiarySummary();
  ...
  function savePetProfile(payload){if(!currentUser||!workspaceId){toast('Увійдіть','error');return Promise.resolve();}showLoading();return db.collection('workspaces').doc(workspaceId).collection('dogs').doc('primary').set(Object.assign({},currentPet||{},payload,{updatedAt:firebase.firestore.FieldValue.serverTimestamp()}),{merge:true}).then(function(){toast('Збережено ✓','success');}).catch(function(e){console.error(e);toast('Помилка','error');}).finally(function(){hideLoading();});}
  function addEvent(payload){if(!currentUser||!workspaceId){toast('Увійдіть','error');return Promise.resolve();}var data={eventType:payload.eventType,byUid:currentUser.uid,byName:currentUser.displayName||'Я',note:payload.note||'',timeLabel:payload.timeLabel||nowTime(),createdAt:firebase.firestore.FieldValue.serverTimestamp()};if(payload.value)data.value=payload.value;return db.collection('workspaces').doc(workspaceId).collection('events').add(data).then(function(){toast('Додано ✓','success');haptic();}).catch(function(e){console.error(e);toast('Помилка','error');});}
  ...
  function ensureWorkspaceForUser(user){return db.collection('users').doc(user.uid).get().then(function(udoc){if(udoc.exists&&udoc.data().workspaceId){workspaceId=udoc.data().workspaceId;return db.collection('workspaces').doc(workspaceId).get().then(function(wdoc){workspaceData=wdoc.exists?wdoc.data():null;});}var wsRef=db.collection('workspaces').doc();workspaceId=wsRef.id;var code=Math.random().toString(36).slice(2,8).toUpperCase();workspaceData={name:(user.displayName||'Мій').split(' ')[0],ownerId:user.uid,inviteCode:code};return wsRef.set(Object.assign({},workspaceData,{createdAt:firebase.firestore.FieldValue.serverTimestamp()})).then(function(){return db.collection('users').doc(user.uid).set({uid:user.uid,email:user.email||'',displayName:user.displayName||'',photoURL:user.photoURL||'',role:'owner',workspaceId:workspaceId},{merge:true});}).then(function(){return wsRef.collection('members').doc(user.uid).set({uid:user.uid,email:user.email||'',displayName:user.displayName||'',photoURL:user.photoURL||'',role:'owner',createdAt:firebase.firestore.FieldValue.serverTimestamp()});}).then(function(){return wsRef.collection('dogs').doc('primary').set({name:'',birthDate:'',sex:'хлопчик',breed:'',toiletMode:'pad',weight:'',issues:'',reminders:[],createdAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()});});});}
  ...
  function loginGoogle(){showLoading();return auth.signInWithPopup(googleProvider).catch(function(e){if(e.code==='auth/popup-blocked'||e.code==='auth/popup-closed-by-user')return auth.signInWithRedirect(googleProvider);toast(e.message||'Помилка','error');}).finally(function(){hideLoading();});}
  function logout(){if(unsubEvents)unsubEvents();if(unsubMembers)unsubMembers();if(unsubPet)unsubPet();unsubEvents=unsubMembers=unsubPet=null;return auth.signOut().then(function(){currentUser=null;workspaceId=null;workspaceData=null;currentPet=null;eventsState=[];membersState=[];hide($('appContent'));show($('authScreen'));});}
  ...
  function fetchAIResponse(prompt){
    ...
    var sys='Ти — український кінолог (15р).\n1. Українською.\n2. 4-5 речень.\n3. Кроки.\n4. Без покарань.\n\n'+petInfo;
    return fetch('/api/proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'groq/llama-3.3-70b-versatile',messages:[{role:'system',content:sys},{role:'user',content:prompt}],temperature:0.2,max_tokens:400,stream:false})}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json();}).then(function(data){if(data.choices&&data.choices[0]&&data.choices[0].message){var t=data.choices[0].message.content.trim().replace(/[\u4e00-\u9fff\u3040-\u30ff\uff00-\uffef]/g,'').trim();return t||'Спробуйте конкретніше.';}throw new Error('Empty');}).catch(function(e){console.warn('AI:',e.message);var l=prompt.toLowerCase();if(l.indexOf('сідати')>=0)return '1) Ласощі біля носа. 2) Руку вгору. 3) "Так!"+ласощі.';if(l.indexOf('туалет')>=0||l.indexOf('пелюшк')>=0)return '1) Менше простору. 2) Після сну/їжі на місце. 3) "Так!" одразу.';return (getProgramByAge(getAgeInWeeks(currentPet&&currentPet.birthDate))||{}).tip||'Запитайте конкретніше!';});
  }

  function handleAISubmit(prompt){if(!prompt.trim())return;addChatMessage(prompt,'user');showTyping();fetchAIResponse(prompt).then(function(r){removeTyping();addChatMessage(r,'assistant');}).catch(function(){removeTyping();addChatMessage('Помилка','assistant');});}

  function requestPushPermission(){
    if(!('Notification' in window)){toast('Не підтримується','error');return;}
    Notification.requestPermission().then(function(perm){
      if(perm==='granted'){toast('Увімкнені! 🔔','success');subscribeToPush();}
      else toast('Відхилено','error');
      fillPetForm();
    });
  }
  function subscribeToPush(){
    try{if(!firebase.messaging)return;var messaging=firebase.messaging();navigator.serviceWorker.getRegistration().then(function(reg){if(!reg)return;return messaging.getToken({vapidKey:'BFvGyG-w5R68xO2RS6gQbYSyAPQaviGnVsHedxjzXajvxg1OUdL1Xe6e4M38j0mewG-Yt3qKgbUnMHmf98PaCiA',serviceWorkerRegistration:reg});}).then(function(token){if(token&&currentUser&&workspaceId)db.collection('workspaces').doc(workspaceId).collection('members').doc(currentUser.uid).update({pushToken:token});}).catch(function(e){console.warn('Push:',e);});}catch(e){console.warn('Push:',e);}
  }
  ...
  function bindOnboarding(){
    $('obNext1')&&$('obNext1').addEventListener('click',function(){if(!$('obName').value.trim()){toast('Ім\'я!','error');return;}setOnboardingStep(2);});
    ...
    $('obFinish')&&$('obFinish').addEventListener('click',function(){showLoading();savePetProfile({name:$('obName').value.trim(),birthDate:$('obBirthDate').value,sex:$('obSex').value,breed:$('obBreed').value.trim()}).then(function(){localStorage.setItem('dc_onboarded','true');hideOnboarding();toast('Готово! 🎉','success');queueRender();}).finally(function(){hideLoading();});});
  }
  ...
  function bindEvents() {
    ...
    // Reminders
    $('addReminderBtn')&&$('addReminderBtn').addEventListener('click',function(){
      var type=$('reminderType')&&$('reminderType').value;
      var date=$('reminderDate')&&$('reminderDate').value;
      if(!type||!date){toast('Оберіть тип і дату','error');return;}
      var reminders=(currentPet&&currentPet.reminders)||[];
      reminders.push({label:type,nextDate:date});
      savePetProfile({reminders:reminders}).then(function(){if($('reminderDate'))$('reminderDate').value='';});
    });
    ...
    var aiInput=$('aiInput');if(aiInput){aiInput.addEventListener('input',function(){aiInput.style.height='auto';aiInput.style.height=Math.min(aiInput.scrollHeight,100)+'px';});aiInput.addEventListener('keydown',function(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();$('aiForm').dispatchEvent(new Event('submit'));}});}
