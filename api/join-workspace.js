...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (existingMember.exists) {
      ...
      const currentWorkspaceId = userDoc.exists ? userDoc.data().workspaceId : '';
      const update = { workspaceId };
      if (currentWorkspaceId && currentWorkspaceId !== workspaceId) {
        update.previousWorkspaceIds = admin.firestore.FieldValue.arrayUnion(currentWorkspaceId);
      }
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (currentWorkspaceId && currentWorkspaceId !== workspaceId) {
        profile.previousWorkspaceIds = admin.firestore.FieldValue.arrayUnion(currentWorkspaceId);
      }
