...
function renderDailyPlan() {
  const list = $('dailyItems'), badge = $('dailyProgressBadge');
...
function renderKpis() {
  ...
  $('ringPct').textContent = `${Math.round(success / total * 100)}%`;
  const ring = $('ringFill'); if (ring) ring.style.strokeDashoffset = String(264 - (264 * success / total));
}
...
function renderSuggestion() {
  ...
  if (toilet.length < 4) { text.textContent = getProgramByAge(getAgeInWeeks(currentPet?.birthDate))?.tip || 'Записуйте події кілька днів поспіль.'; return; }
...
function renderCourses() {
  const grid = $('courseGrid'), viewer = $('selectedCourse');
  ...
  viewer.innerHTML = `<div class="course-detail"><h3>${course.title}</h3><p>${course.description}</p><h4>Кроки</h4><ul>${course.steps.map(s => `<li>${s}</li>`).join('')}</ul></div>`;
}
...
function renderFeed(targetId = 'recentLogs') {
  ...
  list.innerHTML = eventsState.slice(0, 20).map(item => {
    ...
    return `<div class="feed-item" data-event-id="${item.id}"><div><strong>${conf.icon} ${conf.label}</strong><div class="meta">${createdToDate(item.createdAt) ? createdToDate(item.createdAt).toLocaleString('uk') : ''}${item.note ? ` · ${item.note}` : ''}</div></div><button type="button" class="btn-sm" data-delete-event="${item.id}">Видалити</button></div>`;
  }).join('');
...
function renderChart(canvasId) {
  ...
  const primary = css.getPropertyValue('--primary').trim(), danger = css.getPropertyValue('--danger').trim(), faint = css.getPropertyValue('--faint').trim(), line = css.getPropertyValue('--line').trim();
...
async function ensureWorkspaceForUser(user) {
  ...
  const wsRef = db.collection('workspaces').doc(); workspaceId = wsRef.id; const inviteCode = Math.random().toString(36).slice(2, 8).toUpperCase(); workspaceData = { name: `${(user.displayName || 'Мій').split(' ')[0]} Family`, ownerId: user.uid, inviteCode };
...
async function loginGoogle() {
  try {
    ...
    try { await auth.signInWithRedirect(googleProvider); } catch (err) { toast(err.message || 'Login error'); }
  }
}
...
async function logoutGoogle() {
  ...
  setVisible($('appContent'), false); setVisible($('authScreen'), true);
}
...
function bindEvents() {
  ...
  $('petProfileForm')?.addEventListener('submit', async e => { e.preventDefault(); await savePetProfile({ name: $('petName').value.trim(), birthDate: $('petBirthDate').value, sex: $('petSex').value, breed: $('petBreed').value.trim(), toiletMode: $('petToiletMode').value }); toast('Профіль збережено'); });
  $('copyInviteBtn')?.addEventListener('click', async () => { if (!workspaceData?.inviteCode) return; try { await navigator.clipboard.writeText(workspaceData.inviteCode); toast('Код скопійовано'); } catch { toast('Не вдалося скопіювати'); } });
  $('joinWorkspaceForm')?.addEventListener('submit', async e => { e.preventDefault(); try { await joinWorkspaceByInvite($('inviteCodeInput').value); $('inviteCodeInput').value = ''; toast('Ви приєдналися'); } catch (err) { toast(err.message || 'Не вдалося приєднатися'); } });
}
...
function bootAuth() {
  auth.onAuthStateChanged(async user => {
    ...
    try { await ensureWorkspaceForUser(currentUser); subscribePet(); subscribeMembers(); subscribeEvents(); renderAll(); } catch (e) { toast(e.message || 'Помилка запуску'); }
  });
}
